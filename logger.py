...
def init_logger(logger_name):
    if logger_name not in Logger.manager.loggerDict:
        ...
        # handler all
        handler1 = TimedRotatingFileHandler('/root/logs/weixin/all.log', when='D', interval=1, backupCount=7)
        handler1.setFormatter(formatter)
        handler1.setLevel(logging.DEBUG)
        logger1.addHandler(handler1)
        # handler error
        handler2 = TimedRotatingFileHandler('/root/logs/weixin/error.log', when='D', interval=1, backupCount=7)
